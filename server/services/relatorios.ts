...
export async function getReportKPIs(userId: string, startDate: string, endDate: string) {
  ...
  for (const a of confirmed) {
    revenueByProfessional[a.professionalName] = (revenueByProfessional[a.professionalName] || 0) + a.servicePrice
  ...
  return {
    ...
    totalRevenue,
    averageTicket,
    retentionRate: uniqueClients.size > 0
      ? Math.round((recurringPhones.size / uniqueClients.size) * 100)
      : 0,
    ...
    revenueByProfessional: Object.entries(revenueByProfessional).map(([name, total]) => ({ name, total })),
